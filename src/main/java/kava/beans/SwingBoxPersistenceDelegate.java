...
class SwingBoxPersistenceDelegate extends kava.beans.PersistenceDelegate
{
	@Override
    protected kava.beans.Expression instantiate(Object oldInstance, kava.beans.Encoder enc) {
		return new kava.beans.Expression(oldInstance, oldInstance.getClass(),
    ...
    protected void initialize(Class<?> type, Object oldInstance,
		...
		kava.beans.Expression getterExp = new kava.beans.Expression(box.getAlignmentX(), box,
				"getAlignmentX", null);
		try {
			...
			if (null == targetVal) {
				if (null != newVal) {
					// Set to null
					kava.beans.Statement setterStm = new kava.beans.Statement(oldInstance, "setAlignmentX",
							new Object[] { null });
				...
				if (!pd.mutatesTo(targetVal, newVal)) {
					kava.beans.Statement setterStm = new Statement(oldInstance,
