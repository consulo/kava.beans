...
class UtilMapPersistenceDelegate extends DefaultPersistenceDelegate
{
    ...
    protected void initialize(Class<?> type, Object oldInstance,
		...
		for (Iterator i = keySet.iterator(); i.hasNext();) {
			Object key = i.next();
			kava.beans.Expression getterExp = new kava.beans.Expression(oldInstance, "get",
					new Object[] { key});
			try {
				...
				if (null == targetVal) {
					if (null != newVal) {
						// Set to null
						kava.beans.Statement setterStm = new kava.beans.Statement(oldInstance, "put",
								new Object[] { key, null });
					...
					if (!pd.mutatesTo(targetVal, newVal)) {
						kava.beans.Statement setterStm = new Statement(oldInstance, "put",
