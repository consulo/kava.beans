...
class AwtContainerPersistenceDelegate extends AwtComponentPersistenceDelegate {
    ...
    protected void initialize(Class<?> type, Object oldInstance,
		...
		kava.beans.Expression getterExp = null;
		for (int i = 0; i < count; i++) {
			getterExp = new kava.beans.Expression(container.getComponent(i), container,
			...
			try {
				...
				if (null == targetVal) {
					if (null != newVal) {
						// Set to null
						kava.beans.Statement setterStm = new kava.beans.Statement(oldInstance, "add",
								new Object[] { null });
					...
					if (!pd.mutatesTo(targetVal, newVal)) {
						kava.beans.Statement setterStm = new Statement(oldInstance, "add",
								new Object[] { oldVal });
						enc.writeStatement(setterStm);
					}
				}
