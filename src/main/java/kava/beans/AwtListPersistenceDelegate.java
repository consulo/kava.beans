...
class AwtListPersistenceDelegate extends DefaultPersistenceDelegate
{
    ...
    protected void initialize(Class<?> type, Object oldInstance,
		...
		kava.beans.Statement setterStm = new kava.beans.Statement(oldInstance, "setSize",
		...
		for (int i = 0; i < count; i++) {
			getterExp = new kava.beans.Expression(list, "getItem", new Object[] { i });
			try {
				...
				if (null == targetVal) {
					if (null != newVal) {
						// Set to null
						setterStm = new kava.beans.Statement(oldInstance, "add",
								new Object[] { null });
						enc.writeStatement(setterStm);
					}
				} else {
