...
class SwingJTabbedPanePersistenceDelegate extends DefaultPersistenceDelegate
{
    ...
    protected void initialize(Class<?> type, Object oldInstance,
    	...
    	for (int i = 0; i < count; i++) {
			kava.beans.Expression getterExp = new kava.beans.Expression(pane.getComponentAt(i), pane,
			...
			try {
                ...
                if (null == targetVal) {
                    if (null != newVal) {
                        // Set to null
                    	kava.beans.Statement setterStm = new kava.beans.Statement(oldInstance, "addTab",
                                new Object[] { pane.getTitleAt(i), oldVal });
                        enc.writeStatement(setterStm);
                    }
                } else {
                    kava.beans.Statement setterStm = new Statement(oldInstance, "addTab",
                            new Object[] { pane.getTitleAt(i), oldVal });
                    enc.writeStatement(setterStm);
                }
            } catch (Exception ex) {
