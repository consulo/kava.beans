...
class SwingDefaultComboBoxModelPersistenceDelegate extends DefaultPersistenceDelegate
{
    ...
    protected void initialize(Class<?> type, Object oldInstance,
		...
		for (int i = 0; i < count; i++) {
			getterExp = new kava.beans.Expression(model, "getElementAt", new Object[] { i });
			try {
				 ...
				 */
				kava.beans.Statement setterStm = null;
				if (null == targetVal) {
					if (null != newVal) {
						// Set to null
						setterStm = new kava.beans.Statement(oldInstance, "addElement",
								new Object[] { null });
