...
class AwtComponentPersistenceDelegate extends DefaultPersistenceDelegate
{
    ...
    static void writeProperty(Object oldInstance, Object newInstance,
		...
		kava.beans.Expression getterExp = new kava.beans.Expression(oldInstance, builder
		...
		try {
			...
			if (null == targetVal) {
				if (null != newVal) {
					// Set to null
					builder.delete(0, builder.capacity());
					kava.beans.Statement setterStm = new kava.beans.Statement(oldInstance, builder
				...
				if (!pd.mutatesTo(targetVal, newVal)) {
					builder.delete(0, builder.capacity());
					kava.beans.Statement setterStm = new Statement(oldInstance, builder
							.append("set").append(property).toString(),
							new Object[] { oldVal });
					enc.writeStatement(setterStm);
				}
