...
class SwingJFramePersistenceDelegate extends DefaultPersistenceDelegate
{
    ...
    protected void initialize(Class<?> type, Object oldInstance,
    	...
    	if(count != 0) {
    		kava.beans.Expression getterExp = new kava.beans.Expression(frame,
					"getContentPane", null);
    		try {
                ...
                if (null == targetVal) {
                    if (null != newVal) {
                        // Set to null
                        kava.beans.Statement setterStm = new kava.beans.Statement(oldInstance, "setContentPane",
								new Object[] { null });
                    ...
                    if (!pd.mutatesTo(targetVal, newVal)) {
                        kava.beans.Statement setterStm = new Statement(oldInstance, "setContentPane",
                                new Object[] { oldVal });
                        enc.writeStatement(setterStm);
                    }
                }
